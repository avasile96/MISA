...
def segmentEM(volume_dir,labels_dir,mask_dir,init_mode,mode,export,atlas=None):
    ...
    labels                   = np.array(sitk.GetArrayFromImage(sitk.ReadImage(labels_dir))) # transformed labels
    mask                     = np.array(sitk.GetArrayFromImage(sitk.ReadImage(mask_dir))) # testing mask
    ...
    feature_vector_indices   = [i for i,j in enumerate(feature_vector) if j.any()] # indices of nonzero elements
    ...
    if (init_mode=='kmeans'):
        ...
        if (atlas=='training'):
            ...
            # bin_mask         = np.array(sitk.GetArrayFromImage(sitk.ReadImage('../data/testing-set/testing-mask/1'+index+'_1C.nii.gz')))
            
            # propg_labels = probatlas_CSF + probatlas_WM*2 + probatlas_GM*3
            propg_labels = propg_labels.reshape(-1,1)
            propg_labels_indices       = [i for i,j in enumerate(propg_labels) if j.any()]
            ATpredict                  = propg_labels[propg_labels_indices]
    ...
    if KM_CSF.any() == False:
        ...
        prob_distr_0 = np.array((prob_WM , prob_GM))
        ...
        for i in range(max_steps):
            ...
            weights_WM   = (prob_WM  * PDF_WM) /((prob_WM * PDF_WM) + (prob_GM * PDF_GM))
            weights_GM   = (prob_GM  * PDF_GM) /((prob_WM * PDF_WM) + (prob_GM * PDF_GM))
            weights      = np.concatenate((weights_WM.reshape(-1,1),weights_GM.reshape(-1,1)),axis=1)
            ...
            prob_distr_1 = np.array((prob_WM , prob_GM))
        ...
        prob_distr_0 = np.array((prob_CSF, prob_GM))
        ...
        for i in range(max_steps):
            ...
            weights_CSF  = (prob_CSF * PDF_CSF)/((prob_CSF * PDF_CSF) + (prob_GM * PDF_GM))
            weights_GM   = (prob_GM  * PDF_GM) /((prob_CSF * PDF_CSF) + (prob_GM * PDF_GM))
            weights      = np.concatenate((weights_CSF.reshape(-1,1),weights_GM.reshape(-1,1)),axis=1)
            log_B        = sum((np.log(sum(weights))))
            ...
            prob_GM      = counts[2] / feature_vector_nonzero.shape[0]
            prob_distr_1 = np.array((prob_CSF , prob_GM))
        ...
        prob_distr_0 = np.array((prob_CSF, prob_WM))
        ...
        for i in range(max_steps):
            ...
            weights_CSF  = (prob_CSF * PDF_CSF)/((prob_CSF * PDF_CSF) + (prob_WM * PDF_WM))
            weights_WM   = (prob_WM  * PDF_WM) /((prob_CSF * PDF_CSF) + (prob_WM * PDF_WM))
            weights      = np.concatenate((weights_CSF.reshape(-1,1),weights_WM.reshape(-1,1)), axis = 1)
            log_B        = sum((np.log(sum(weights))))
            ...
            if (len(counts)==1):
                counts = [1e-7,counts[0]]
                
            prob_CSF     = counts[0] / feature_vector_nonzero.shape[0]
            prob_WM      = counts[1] / feature_vector_nonzero.shape[0]
            prob_distr_1 = np.array((prob_CSF, prob_WM))
    ...
    if (mode=='atlas'):
        ...
        if KM_CSF.any() == False:
            ...
            max_index          = np.argmax(np.concatenate((prob_WM, prob_GM),axis=3),axis=3)+1
        if KM_WM.any() == False:
            ...
            # Assign Label Matching Max Class Probability
            max_index          = np.argmax(np.concatenate((prob_CSF, prob_GM),axis=3),axis=3)+1
        if KM_GM.any() == False:
            predicted_mask_EM_CSF                         = np.zeros((masked_volume.reshape(-1,1)).shape)
            predicted_mask_EM_CSF[feature_vector_indices] = weights[:,0].reshape(-1,1)
            predicted_mask_EM_CSF                         = np.reshape(predicted_mask_EM_CSF, masked_volume.shape)
            ...
            # Assign Label Matching Max Class Probability
            max_index          = np.argmax(np.concatenate((prob_CSF, prob_WM),axis=3),axis=3)+1
        else:
            predicted_mask_EM_CSF                         = np.zeros((masked_volume.reshape(-1,1)).shape)
            predicted_mask_EM_CSF[feature_vector_indices] = weights[:,0].reshape(-1,1)
            predicted_mask_EM_CSF                         = np.reshape(predicted_mask_EM_CSF, masked_volume.shape)
    ...
    if (export=='return'):
        if len(predicted_mask_EM.shape) != len(labels.shape):
            return predicted_mask_EM.reshape(labels.shape), dice_metric(predicted_mask_EM.reshape(labels.shape),labels,3)
        else:
