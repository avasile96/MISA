...
for i = 1:1 % numel(list)
    ...
    imwrite(seg.*ROI/max(max(seg.*ROI)), sprintf('%s%s_q_%d_iter_%d_basis_%d%s',BaseName_save,'seg_res',q,iterNum,basis_order,'.png'));

    subplot(143),imshow(b.*ROI,[]),title('bias field');
    imwrite(b.*ROI/max(max(b.*ROI)), sprintf('%s%s_q_%d_iter_%d_basis_%d%s',BaseName_save,'bf',q,iterNum,basis_order,'.png'));

    subplot(144),imshow(uint8(img_bc.*ROI),[]),title('bias corrected');
    imwrite(img_bc.*ROI/max(max(img_bc.*ROI)), sprintf('%s%s_q_%d_iter_%d_basis_%d%s',BaseName_save,'bf_corr',q,iterNum,basis_order,'.png'));
end
